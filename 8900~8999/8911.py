...
# 1 초	128 MB	3767	2026	1579	52.739%
# 문제
...
for m in move:
    ...
    for c in m:
        if c == 'R' :
            ...
            mp[0].append(mp[0][p] + dx[d])
            mp[1].append(mp[1][p] + dy[d])
            p += 1
        else :
            mp[0].append(mp[0][p] + -1 * dx[d])
            mp[1].append(mp[1][p] + -1 * dy[d])
            p += 1
    print((max(mp[0])-min(mp[0])) * (max(mp[1]) - min(mp[1])))
